...
usage () {
  echo "usage: mltrain.sh [local | train | tune] [gs://]job_and_data_dir [path_to/]<input_file>.csv
                  [--data-type your_date_type|wolf_groups]
                  [--delimiter <delim>]
                  [--use-optimized]
                  [--headers]
...
#custom environment image uri:
IMAGE_URI=gcr.io/palringo-client/recommendation_bespoke_container:tf_rec_ur
...
elif [[ ${TRAIN_JOB} == "train_custom" ]]; then
